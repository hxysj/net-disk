<template>
    <div class="top">
        <div class="input-group">
            ...
            <input type="text" placeholder="支持模糊搜索" v-model="searchForm.value" class="form-control" @keyup.enter="loadDataList()">
        </div>
        <select class="form-select form-select-sm" aria-label=".form-select-lg example" v-model="searchForm.status">
            <option :value="true" selected>启用</option>
            <option :value="false">禁用</option>
        </select>
        <button class="btn  btn-primary" type="button" @click="loadDataList(true)">搜索</button>
        <div class="iconfont icon-refresh refresh" @click="loadDataList(false)"></div>
    </div>
    <div class="user-list">
        <table class="table table-hover">
            ...
            <tbody>
                <tr v-for="row in tableData.list">
                    ...
                    <td>{{ row.nickName }}</td>
                    <td>{{ formatFileSize(row.useSpace)}}/{{ formatFileSize(row.totalSpace) }}</td>
                    ...
                    <td>
                        <div class="op">
                            <span class="text-primary" @click="changeSpace(row)">分配空间</span>
                            <span v-if="row.status" class="text-primary" @click="changeStatus(row)">禁用</span>
                            <span style="color: #569261;" v-else @click="changeStatus(row)">启用</span>
                        </div>
                    </td>
                </tr>
            </tbody>
        </table>
        <MessageModal ref="messageModal" @submit="submitChangeStatus"></MessageModal>
        <!-- 更改用户空间的模态框 -->
        <div class="modal" :style="{display:showChangeSpace? 'block':''}" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">分配空间</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"  @click="closeSpace"></button>
                </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" @click="closeSpace">取消</button>
                    <button type="button" class="btn btn-primary" :class="[newSpace ? '':'disabled']" @click="submitSpace">确定</button>
                </div>
                </div>
            </div>
        </div>
