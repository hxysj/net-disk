    ...
    <div class="">
        <div class="top">
            <div class="btn btn-primary" @click="clickAddFile">
                ...
                上传
                <input type="file" ref="fileInput" :accept="fileAccept" style="display: none;" @change="addFile" />
            </div>
            <div class="btn btn-success " @click="addNewFolder" v-if="category === 'all'">
                <span class="iconfont icon-folder-add"></span>
                新建文件夹
            </div>
            <div class="btn btn-danger" :class="[!haveSelect?'disabled':'']" @click="delAllCheck">
                <span class="iconfont icon-del"></span>
                批量删除
            </div>
            <div class="btn btn-warning" :class="[!haveSelect?'disabled':'']" @click="moveFolderPath">
            ...
            <div class="input-group">
                ...
                <input 
                    ...
                    @keyup.enter="search"
                >
                <button class="btn btn-outline-secondary" :class="[fileNameFuzzy ? '':'disabled']" type="button" id="button-addon2" @click="search">搜索</button>
            </div>
                ...
                <template #tableBody>
                    <tr class="table-file" v-if="newFolder.newEdit">
                        ...
                        <td class="edit container d-flex flex-nowarp">
                            <div class="col-7">
                                <input type="text" ref="editIpt" v-model="newFolder.fileName" class="form-control">
                            </div>
                            <span :class="['iconfont','icon-right','col-1','btn','btn-primary',newFolder.fileName?'':'disabled']" @click="submitEditName(true)"></span>
                            <span class="iconfont icon-close2 col-1 btn btn-light" @click="closeEditName(true)"></span>
                        </td>
                        <td></td>
                        <td></td>
                    </tr>
                    ...
                    >
                        ...
                        <td class="filename" @click="changeFolder($event,data)">
                            <div class="filename-left">
                                <!-- 如果是视频类型，则有封面，不使用图标 -->
                                <template v-if="(data.fileType === 3 || data.fileType === 1) && data.status === 2">
                                    <FileIcon
                                ...
                                <div class="edit container flex-nowarp" v-else>
                                    <div class="col-10 input-group">
                                        ...
                                        >
                                        <span class="input-group-text" v-if="currentData.fileSuffix">{{ currentData.fileSuffix }}</span>
                                    </div>
                                    <span :class="['iconfont','icon-right','col-1','btn','btn-primary',data.fileName?'':'disabled']" @click="submitEditName(false,data)"></span>
                                    <span class="iconfont icon-close2 col-1 btn btn-light" @click="closeEditName(false,data)"></span>
                                </div>

                                <span v-if="data.status === 0" class="transfer transfer-status">转码中</span>
                                <span v-if="data.status === 1" class="transfer transfer-fail">转码失败</span>
                            </div>
                            <div v-show="data.showOp" class="filename-right">
                                <!-- {{ data.status }} -->
                                <span class="iconfont icon-share1 right-item" @click="showShareFile(data)" v-if="data.status == 2">分享</span>
                                <span class="iconfont icon-download right-item" v-if="data.folderType === 0" @click="download(data)" v-show="data.status ==2">下载</span>
                                <span class="iconfont icon-del right-item" @click="deleteFile(data)">删除</span>
                                <span class="iconfont icon-edit right-item" @click="editName(data)">重命名</span>
                                <span class="iconfont icon-move right-item" @click="moveOneFile(data)">移动</span>
                            </div>
                        </td>
                    ...
                    <div class="not-data">
                        ...
                        <div class="op-tip">
                            ...
                            <div class="op-tip-item" @click="addNewFolder">
                                <div><img src="/src/assets/icon-image/folder.png" alt=""></div>
                                <div>新建目录</div>
                            </div>
                        </div>
                    </div>
...
<script setup lang="ts">
    ...
    const submitEditName:(isnewDir:boolean,data?:dataListItem) =>void =async (isnewDir,data)=>{
        let name = ''
    ...
    // 取消修改
    const closeEditName:(isnewDir:boolean,data?:dataListItem) => void =(isnewDir = false,data)=>{
        if(isnewDir){
            newFolder.value.newEdit = false
            newFolder.value.fileName = ''
        }else{
    ...
    // 确定移动文件到对应目录
    const moveFolderDone:(obj:{pid:string,list:string[]}) =>Promise<void> = async (obj)=>{
    ...
    const changeFolder:(e:Event,data:dataListItem)=>void = (e,data)=>{
        // 点击的不是目录或文件
        if(e.target && (e.target as HTMLElement).className && (e.target as HTMLElement).className !== 'filename'){
            return
        }
        // 文件类型是目录
    ...
    const download =async (file:dataListItem)=>{
        ...
        window.location.href = baseurl + api.download + '/' + result.data.data + '/' + result.data.fileName
    }
    // ----------------------------------------------------------------------
    // 搜索绑定的值
