                        ...
                        <div class="userinfo">
                            ...
                            <div class="info-value">
                                <div class="info-item">
                                    ...
                                    <div class="info-time">分享于 {{shareInfo.shareTime.split('.')[0].replace('T',' ')}}</div>
                                </div>
                                <div class="info-text">分享文件 {{shareInfo.fileName}}</div>
                            </div>
                        </div>
                        <div class="">
                            <button v-if="shareInfo.currentUser"  class="btn btn-primary"  style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;" @click="cancelShare()">
                                <span class="iconfont icon-cancel"></span>
                                取消分享
                            </button>
                            <button class="btn btn-primary" :class="[selectIdList.length!=0?'':'disabled']" v-else style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;" @click="saveAllFOlder">
                            ...
                            >
                                ...
                                <td class="filename" @click="changeFolder($event,data)">
                                    ...
                                    <div v-show="data.showOp" class="filename-right">
                                        <span class="iconfont icon-download right-item" v-if="data.folderType === 0" @click="download(data)">下载</span>
                                        <span class="iconfont icon-import right-item" v-if="data.showOp && !shareInfo.currentUser" @click="saveToMyPan(data)">保存到我的网盘</span>
                                    </div>
                                </td>
...
<script setup lang="ts">
    ...
    const getShareInfo = async ( ) =>{
        ...
        if(localStorage.getItem('token')){
            if(parseToken(localStorage.getItem('token') as string).uid === shareInfo.value.userId){
    ...
    const download = async(file:dataListItem)=>{
        ...
        window.location.href =baseurl + api.download + '/' + result.data.data + '/' + file.fileName
    }
    // ----------------------------------------------------------------------
    ...
    // 确定移动文件到对应目录
    const moveFolderDone:(obj:{pid:string,list:string[]}) =>void =async (obj)=>{
