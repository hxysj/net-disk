    ...
    <div class="login">
        <div class="container">
            <div class="row">
                ...
                <div class="right">
                    <form ref="submitForm" class="needs-validated" @submit.prevent="onSumbit">
                        ...
                        <template  v-if="opType !== 2">
                            ...
                            <div class="form-group" v-show="opType === 1">
                                <div class="input-group">
                                    ...
                                    >
                                    <div class="input-group-prepend">
                                        <span class="btn btn-primary" :class="[is_disable_email_code?'disabled':'']" id="addon-wrapping" @click="get_email_code">
                                            {{ email_code_btn }}
                                        </span>
                                    </div> 
                                    <div class="invalid-feedback">
                            ...
                            <div class="form-group">
                                <div class="input-group">
                                    ...
                                    >
                                    <div class="input-group-prepend">
                                        <span class="input-group-text code-text" id="addon-wrapping" @click="refesh_code">
                                            <SIdentify :identifyCode="identifyCode" ref="sidentify"></SIdentify>
                                        </span>
                                    </div> 
                                    <div class="invalid-feedback">
                                        {{ errors.code || '错误信息' }}
                            ...
                            <div class="form-group" v-show="opType === 0">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="gridCheck" v-model="rememberMe">
                            ...
                            <div class="form-group" v-if="opType === 0">
                                <div class="form-other">
                                    <a href="#" class="btn-primary" @click="changeStatus(2)">忘记密码？</a>
                                    <a href="#" class="btn-primary" @click="changeStatus(1)">未有账号？</a>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary login-btn" v-if="opType === 0" >登陆</button>
                                <button type="submit" class="btn btn-primary login-btn" v-else-if="opType === 1">注册</button>
                            </div>    
                        </template>
                        <template v-else>
                            <!-- 找回密码 -->
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary login-btn">确定修改</button>
                            </div>
                        </template>
                    </form>
                </div>
...
<script setup lang="ts">
    ...
    const randomNum:(min:number,max:number)=>number =(min, max)=>{
        return Math.floor(Math.random() * (max - min) + min)
    }
     // 生成随机验证码
    const makeCode:(list:string, num:number) => void =(list, num)=> {
        var numCode = ""
        for (let i = 0; i < num; i++) {
            numCode += list[
    ...
    const validateFrom:(type:string)=>boolean = (type) => {
        ...
        if((type === 'pwd2' || type ==='all') && (opType.value === 1 || opType.value === 2)){
        ...
        if(type === 'code' || type === 'all'){
            ...
            }else if(code.value !== identifyCode.value){
                errors.value.code = '验证码错误！'
            }else{
                delete errors.value.code
            }
        }
        if((type === 'email' || type==='all')  && (opType.value === 1 || opType.value === 2)){
        ...
        if((type === 'emailCode' || type === 'all')  && (opType.value === 1 || opType.value === 2)){
            if(!emailCode.value){
