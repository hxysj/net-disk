<template>
  <div class="window">
    <div
      class="window-mask modal"
      tabindex="-1"
      style="display: block"
      @click="close($event)"
    >
      <div
        class="modal-dialog window-content"
        :style="{
          'max-width': windowContentWidth + 'px',
...
const close = (e: Event) => {
  if (
    ![...(e.target as HTMLElement).classList].includes("modal") &&
    ![...(e.target as HTMLElement).classList].includes("close")
  ) {
    return;
  }
  emit("close");
