...
<script setup lang="ts">
    ...
    const imageUrl = computed(()=>{
        return baseurl+fileInfo?.value?.fileCover?.replace('_.','.').replace('cover','file')
    })
    // ----------------------------------------------------------------------------
    //文件信息
    ...
    const openPreview = (data:dataListItem|fileListItem,showPort:number) =>{
        ...
        if(data.fileCategory === 3){
            ...
            let _createDownloadUrl = FILE_URL_MAP[showPort as file_url_map_key].createDownloadUrl
            ...
            if(showPort == 0){
                ...
                _createDownloadUrl = _createDownloadUrl +'/' + data.fileId + '/' + data.userId
                 // 临时
                // _url = '/public/vedio.mp4'
            }else if(showPort == 2){
                _url = _url + '/' + data.fileId
